...
% constants
global NPI NPJ Dt Cmu sigmak dudx
% variables
global x x_u y y_v SP Su F_u F_v mut rho u uplus tw Istart Iend ...
    Jstart Jend b aE aW aN aS aP k k_old eps E2 beta_star sigmak  ...
    sigmaw gamma1 beta1 Omega mu LARGE
...
for I = Istart:Iend
    ...
    for J = Jstart:Jend
        ...
        Dw = mut(I-1,J)*mut(I,J)/sigmak/(mut(I-1,J)*(x(I) - x_u(i)) + ...
            mut(I,J)*(x_u(i)-x(I-1)))*AREAw + mu(I,J)/(x(I)-x(I-1))*AREAw;
        De = mut(I,J)*mut(I+1,J)/sigmak/(mut(I,J)*(x(I+1) - x_u(i+1)) + ...
            mut(I+1,J)*(x_u(i+1)-x(I)))*AREAe + mu(I,J)/(x(I)-x(I-1))*AREAe;
        Ds = mut(I,J-1)*mut(I,J)/sigmak/(mut(I,J-1)*(y(J) - y_v(j)) + ...
            mut(I,J)*(y_v(j)-y(J-1)))*AREAs + mu(I,J)/(x(I)-x(I-1))*AREAs;
        Dn = mut(I,J)*mut(I,J+1)/sigmak/(mut(I,J)*(y(J+1) - y_v(j+1)) + ...
            mut(I,J+1)*(y_v(j+1)-y(J)))*AREAn + mu(I,J)/(x(I)-x(I-1))*AREAn;
        
        % The source terms
        if J==2 || J==NPJ+1  %Klopt nog niet --> Randwaarden
            SP(I,J) = -rho(I,J)*Cmu^0.75*k(I,J)^0.5*uplus(I,J)/(0.5*AREAw)*AREAs*AREAw;
            Su(I,J) = tw(I,J)*0.5*(u(i,J) + u(i+1,J))/(0.5*AREAw)*AREAs*AREAw;
        else
            SP(I,J) = -beta_star.*rho(I,J).*k(I,J).*Omega(I,J);
            Su(I,J) = 2.0.*mut(I,J)*E2(I,J)-2/3.*rho(I,J).*k(I,J).*dudx(I,J).*eq(I,J); %inproduct kronicker delta werkt wellicht niet
        end
