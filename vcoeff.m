...
for I = Istart:Iend
    ...
    for J = Jstart:Jend
        ...
        if (I == ceil((NPI+1)/10) && j < ceil((NPJ+1)/3))     % left of baffle #1
            aE(I,j) = 0;
            SP(I,j)=0;
        end
        if (I == ceil((NPI+1)/10 + 15)   && j < ceil((NPJ+1)/3))     % right of baffle #1
            aW(I,j) = 0;
            SP(I,j)=0;

        end
        
        if (I == ceil((NPI+1)/10 + 25) && j < ceil((NPJ+1)/3))     % left of baffle #1
            aE(I,j) = 0;
            SP(I,j)=0;
        end
        if (I == ceil((NPI+1)/10 + 40)   && j < ceil((NPJ+1)/3))     % right of baffle #1
            aW(I,j) = 0;
            SP(I,j)=0;
